...
import { suggestTitles } from "@/lib/gemini"
...
const suggestTitlesSchema = z.object({
  ...
  style: z.enum(["clickbait", "professional", "creative"]).optional(),
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validatedData = suggestTitlesSchema.parse(body)

    // Generate title suggestions
    const result = await suggestTitles(validatedData)
